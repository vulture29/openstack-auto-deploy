...
while read -p "Want to proceed with default config -- allinone?(yes/no) " REPLY1 ; do
  case $REPLY1 in
	...
	if [ $USER_INPUT -eq 1 ]; then
			...
			sed -i 's/^CONFIG_CONTROLLER_HOST/#CONFIG_CONTROLLER_HOST/g' $DEFAULT_CONFIG_PATH
			echo "CONFIG_CONTROLLER_HOST = " $COMPUTE_IP >> $DEFAULT_CONFIG_PATH
			sed -i 's/^CONFIG_NETWORK_HOST/#CONFIG_NETWORK_HOST/g' $DEFAULT_CONFIG_PATH
			echo "CONFIG_NETWORK_HOST = " $NETWORK_IP >> $DEFAULT_CONFIG_PATH
			sed -i 's/^CONFIG_COMPUTE_HOST/#CONFIG_COMPUTE_HOST/g' $DEFAULT_CONFIG_PATH
			echo "CONFIG_COMPUTE_HOST = " $COMPUTE_IP >> $DEFAULT_CONFIG_PATH
			sed -i 's/^CONFIG_STORAGE_HOST/#CONFIG_STORAGE_HOST/g' $DEFAULT_CONFIG_PATH
			echo "CONFIG_STORAGE_HOST = " $STORAGE_IP >> $DEFAULT_CONFIG_PATH

			sed -i 's/^CONFIG_NOVA_INSTALL/#CONFIG_NOVA_INSTALL/g' "$DEFAULT_CONFIG_PATH"
			echo "CONFIG_NOVA_INSTALL = " $NOVA_ENABLE >> $DEFAULT_CONFIG_PATH
			sed -i 's/^CONFIG_NEUTREN_INSTALL/#CONFIG_NEUTREN_INSTALL/g' $DEFAULT_CONFIG_PATH
			echo "CONFIG_NEUTREN_INSTALL = " $NEUTREN_ENABLE >> $DEFAULT_CONFIG_PATH
			sed -i 's/^CONFIG_GLANCE_INSTALL/#CONFIG_GLANCE_INSTALL/g' $DEFAULT_CONFIG_PATH
			echo "CONFIG_GLANCE_INSTALL = " $GLANCE_ENABLE >> $DEFAULT_CONFIG_PATH
			sed -i 's/^CONFIG_HORIZON_INSTALL/#CONFIG_HORIZON_INSTALL/g' $DEFAULT_CONFIG_PATH
			echo "CONFIG_HORIZON_INSTALL = " $HORIZON_ENABLE >> $DEFAULT_CONFIG_PATH
			sed -i 's/&CONFIG_CINDER_INSTALL/#CONFIG_CINDER_INSTALL/g' $DEFAULT_CONFIG_PATH
			echo "CONFIG_CINDER_INSTALL = " $CINDER_ENABLE >> $DEFAULT_CONFIG_PATH
			sed -i 's/^CONFIG_SWIFT_INSTALL/#CONFIG_SWIFT_INSTALL/g' $DEFAULT_CONFIG_PATH
			echo "CONFIG_SWIFT_INSTALL = " $SWIFT_ENABLE >> $DEFAULT_CONFIG_PATH
